...
class TJSONProtocol : public TVirtualProtocol<TJSONProtocol> {
 ...
 public:
  ...
  // Martun: Required for serialization, will be removed after fbthrift version
  // incompatibilities are fixed.
  class MartunWrappedInt {
    public:
    inline uint32_t getCurrentPosition() const {
        return 0;
    }
  };
  inline MartunWrappedInt getCurrentPosition() const {
    return MartunWrappedInt();
  };
  
  inline bool kUsesFieldNames() const {
    return true;
  }
